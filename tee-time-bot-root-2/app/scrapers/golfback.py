"""
GolfBack Scraper — Playwright DOM extraction
Covers: Prairie Landing Golf Club
GolfBack is a Vue SPA with UUID-based course routing.
"""
...
GOLFBACK_COURSES = {
    "prairie_landing": {
        "uuid": "dba9546a-1cdf-4c55-8abb-e8bfcb7c6c84",
    },
}


async def search_golfback(course_id: str, date: str, players: int = 4) -> list[dict]:
    """Search a GolfBack-powered course for tee times."""
    config = GOLFBACK_COURSES.get(course_id)
    if not config:
        return []

    url = f"https://golfback.com/#/course/{config['uuid']}?date={date}&qty={players}"
    slots = []
    context = None
    ...
    try:
        ...
        logger.info("GolfBack: %s on %s -> %d slots", course_id, date, len(slots))

    except PlaywrightTimeout:
        logger.error("GolfBack timeout for %s on %s", course_id, date)
    except Exception as e:
        logger.error("GolfBack error for %s: %s", course_id, str(e))
    finally:
        if context:
            try:
                await context.close()
