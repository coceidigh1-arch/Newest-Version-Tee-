"""
Whoosh Scraper — Playwright DOM extraction
Covers: Cantigny Golf
Whoosh is a React SPA booking platform.
"""
...
WHOOSH_COURSES = {
    "cantigny": {
        "base_url": "https://app.whoosh.io/patron/club/cantigny-golf-course/agenda/golf-course/",
    },
}


async def search_whoosh(course_id: str, date: str, players: int = 4) -> list[dict]:
    """Search a Whoosh-powered course for tee times."""
    config = WHOOSH_COURSES.get(course_id)
    if not config:
        return []

    # Whoosh uses date in the URL path or query
    url = f"{config['base_url']}?date={date}"
    slots = []
    context = None
    ...
    try:
        ...
        logger.info("Whoosh: %s on %s -> %d slots", course_id, date, len(slots))

    except PlaywrightTimeout:
        logger.error("Whoosh timeout for %s on %s", course_id, date)
    except Exception as e:
        logger.error("Whoosh error for %s: %s", course_id, str(e))
    finally:
        if context:
            try:
                await context.close()
